...
class SvgUri extends Component {
  ...
  constructor(props) {
    ...
    if (props.source != null) {
      if (props.source == '') {
        this.state = { svgXmlData: '<svg></svg>' };
      } else {
        this.state = { svgXmlData: SVGMap._SVG_MAP[props.source] };
      }
    } else {
  ...
  change(source) {
    if (source == '' || source == null) {
      this.setState({ svgXmlData: '<svg></svg>' });
    } else {
      this.setState({ svgXmlData: SVGMap._SVG_MAP[source] });
    }
  }
  ...
  shouldComponentUpdate(nextProps) {
    if (nextProps.source != null) {
      ...
      if (nextProps.source == '') {
        ...
        svgXmlData = SVGMap._SVG_MAP[nextProps.source];
      }
      if (this.state.svgXmlData != svgXmlData) {
        this.state.svgXmlData = svgXmlData;
        return true;
      }
      if (nextProps.fill != null) {
        return true;
      }
    }
    return false;
  ...
  createSVGElement(node, childs) {
    ...
    switch (node.nodeName) {
      case 'svg':
        ...
        return <Svg key={i} {...componentAtts} fill={componentAtts.fill}>{childs}</Svg>;
      case 'g':
        componentAtts = this.obtainComponentAtts(node, G_ATTS);
        return <G key={i} {...componentAtts} fill={componentAtts.fill}>{childs}</G>;
      case 'path':
        componentAtts = this.obtainComponentAtts(node, PATH_ATTS);
        return <Path key={i} {...componentAtts} fill={componentAtts.fill}>{childs}</Path>;
      case 'circle':
        componentAtts = this.obtainComponentAtts(node, CIRCLE_ATTS);
        return <Circle key={i} {...componentAtts} fill={componentAtts.fill}>{childs}</Circle>;
      case 'rect':
        componentAtts = this.obtainComponentAtts(node, RECT_ATTS);
        return <Rect key={i} {...componentAtts} fill={componentAtts.fill}>{childs}</Rect>;
      case 'line':
        componentAtts = this.obtainComponentAtts(node, LINE_ATTS);
        return <Line key={i} {...componentAtts} fill={componentAtts.fill}>{childs}</Line>;
      case 'defs':
        return <Defs key={i}>{childs}</Defs>;
      case 'linearGradient':
        componentAtts = this.obtainComponentAtts(node, LINEARG_ATTS);
        return <LinearGradient key={i} {...componentAtts} fill={componentAtts.fill}>{childs}</LinearGradient>;
      case 'radialGradient':
        componentAtts = this.obtainComponentAtts(node, RADIALG_ATTS);
        return <RadialGradient key={i} {...componentAtts} fill={componentAtts.fill}>{childs}</RadialGradient>;
      case 'stop':
        componentAtts = this.obtainComponentAtts(node, STOP_ATTS);
        return <Stop key={i} {...componentAtts} fill={componentAtts.fill}>{childs}</Stop>;
      case 'ellipse':
        componentAtts = this.obtainComponentAtts(node, ELLIPSE_ATTS);
        return <Ellipse key={i} {...componentAtts} fill={componentAtts.fill}>{childs}</Ellipse>;
      case 'polygon':
        componentAtts = this.obtainComponentAtts(node, POLYGON_ATTS);
        return <Polygon key={i} {...componentAtts} fill={componentAtts.fill}>{childs}</Polygon>;
      case 'polyline':
        componentAtts = this.obtainComponentAtts(node, POLYLINE_ATTS);
        return <Polyline key={i} {...componentAtts} fill={componentAtts.fill}>{childs}</Polyline>;
      case 'text':
