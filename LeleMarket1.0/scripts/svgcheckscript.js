...
for (let i = 0; i < length; i++) {
    ...
    if (file.indexOf('.svg') != -1) {
        ...
        allSvg.push({ key: '\'' + fileName + '\'', path: './svg/' + file, key1: '\"' + fileName + '\"' });
    }
}
...
for (let i = 0; i < length; i++) {
    ...
    for (let j = 0; j < count; j++) {
        if (allFile[j].src.indexOf(allSvg[i].key) != -1 || allFile[j].src.indexOf(allSvg[i].key1) != -1) {
            isUsed = true;
...
console.log(result);
console.log('檢測到' + result.length + '個無用svg文件，' + usedList.length + '個有用文件');
//搜索 'add_w'
