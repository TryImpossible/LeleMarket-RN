...
class TabBar extends React.PureComponent<TabBarProps, TabBarState> {
  ...
  scrollToIndex(index: number, callback?: () => void) {
    ...
    if (animated) {
      ...
      animations.push(Animated.timing(this.indexValue, { toValue: index, useNativeDriver: false }));
      animations.push(Animated.timing(this.indicatorLeftValue, { toValue: leftValue, useNativeDriver: false }));
      animations.push(Animated.timing(this.indicatorWidthValue, { toValue: widthValue, useNativeDriver: false }));
    }
